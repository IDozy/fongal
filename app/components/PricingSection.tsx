...
const plans: PricingPlan[] = [
  ...
  },
  {
    name: 'Pro',
    price: 79,
    features: ['Up to 500 cows', 'Advanced health tracking', 'Weekly reports', 'AI-powered insights'],
    recommended: true,
  },
  {
    name: 'Enterprise',
    price: 199,
    features: ['Unlimited cows', 'Premium health tracking', 'Daily reports', 'AI-powered insights', '24/7 support'],
  },
];
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex justify-center mb-8">
          <div className="bg-white rounded-full p-1 shadow-md">
            <button
              className={`px-4 py-2 rounded-full ${billingCycle === 'monthly' ? 'bg-green-500 text-white' : 'text-gray-700'}`}
              onClick={() => setBillingCycle('monthly')}
            >
              Monthly
            </button>
            <button
              className={`px-4 py-2 rounded-full ${billingCycle === 'annually' ? 'bg-green-500 text-white' : 'text-gray-700'}`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {plans.map((plan) => (
            <motion.div
              key={plan.name}
              className={`bg-white rounded-lg shadow-lg overflow-hidden ${plan.recommended ? 'border-4 border-green-500' : ''}`}
